...
exports.default = async () => {
    try {
        ...
        // } catch (error) { }
        // 2.10-2.11 升级
        const cronDbFile = path_1.default.join('/ql', 'db/crontab.db');
        const envDbFile = path_1.default.join('/ql', 'db/env.db');
        const appDbFile = path_1.default.join('/ql', 'db/app.db');
        const authDbFile = path_1.default.join('/ql', 'db/auth.db');
        const dependenceDbFile = path_1.default.join('/ql', 'db/dependence.db');
        const crondbExist = await util_1.fileExist(cronDbFile);
        const dependenceDbExist = await util_1.fileExist(dependenceDbFile);
        const envDbExist = await util_1.fileExist(envDbFile);
        const appDbExist = await util_1.fileExist(appDbFile);
        const authDbExist = await util_1.fileExist(authDbFile);
